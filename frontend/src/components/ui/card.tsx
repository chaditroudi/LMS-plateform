...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("rounded-[1.95rem] border border-white/80 bg-card/80 text-card-foreground shadow-[0_22px_56px_-42px_rgba(58,56,37,0.24)] backdrop-blur-2xl transition-all", className)}
      {...props}
    />
  )
);
